...
Demonstration of the coverage capabilities in AFSIM.

The demo is to be run using the startup files designated as 1st, 2nd, 3rd, etc.

Each run will output data to the coverage_data folder.
Coverage data can be viewed in the .csv files.

Each run will also produce a heatmap to be viewed in Wizard, Warlock, or Mystic.
To view: View -> Coverage Overlays -> Load Coverage File -> coverage_data

To turn off grid visibility in Mystic, use either the Visibility Window by 
going to View -> Visibility, or go to Options -> Preferences -> Visibility
and toggle on/off the appropriate grid.

The data the grids output can be modified in the files found in the grids folder.
