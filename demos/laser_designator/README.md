...
# laser_designator demo

Top level run file is run_me.txt.
This scenario:
 * has ground troops with an embedded JTAC (aka forward observer) that lases several enemy APC’s that are coming over a hill. 
 * The JTAC passes the track to a UAV and the UAV flies to the lat/lon of the lased APC.
 * Meanwhile, a laser receiver on the APC senses it is being lased and has a comment window saying that ("I'm being lased!").
 * When within range, the hellfire engages the APC and destroys it. Once destroyed, it returns to its default route.
 * As each APC comes over a hill, each are individually engaged by the UAV as lased by the JTAC.

For other examples of controlling a hellfire missile using AFSIM, see the hellfire demos. 
...
Critical Technology, Export Controlled; (2021 Oct 25). Other requests for this
