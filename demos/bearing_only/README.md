...
This demo shows how to create a 3D track from two bearing-only sensors (with computed range) and provide
  it to a third party platform.

The top level run file is test_run.txt.

The bearing-only tracks provide bearing and elevation angles only (2D).

Two methods of computing track location are provided (modify setup.txt to run either)

   1. platforms/fuser_platform1.txt  
        Provides a script that does the trigonometry to compute the location of the target.
        Unfortunately, the track cannot be filtered because it is not derived from a sensor with sigma error values.

   1. platforms/fuser_platform2.txt  
      Uses the WSF_DIRECTION_FINDER_PROCESSOR to derive target location.

Both examples provide a WsfDraw of the reported track location.
